...
**📚 Research → design:** [`RESEARCH.md`](RESEARCH.md) — primary sources behind each control (AICPA 2025 Criteria Pt. II, GENIUS Act, EU AI Act Art. 14, MiCA/MiFID II, DORA, SEC Rule 206(4)-7), each claim adversarially verified 2-of-3. It also states where our claims **don't** reach: the AICPA mapping is an analogy (those criteria are scoped to stablecoins), AI Act duties bite only *if* high-risk classification applies, a K-of-N quorum improves control but **cannot transfer legal liability**, and without an independent ZK-circuit audit "we have a ZK proof" is not auditor evidence. Three of five research angles produced nothing verifiable and are listed as open questions rather than dressed up as findings.

**🧾 Control evidence:** [amanah-casper-rwa.vercel.app/compliance](https://amanah-casper-rwa.vercel.app/compliance) — the artifact SEC staff ask for under **Advisers Act Rule 206(4)-7**: an **exception report** of every transaction a control refused (named by the control, not a raw error code), the authorised activity that passed every gate, and a one-click JSON export. Generated from chain, not from a database we own. Threat model + residual risk: [`THREAT_MODEL.md`](THREAT_MODEL.md).

**🔍 Don't trust us — verify:** [amanah-casper-rwa.vercel.app/verify](https://amanah-casper-rwa.vercel.app/verify) re-runs our cryptography **in your own browser** against the exact bytes the Casper contracts accepted: the Pedersen+Schnorr **proof-of-solvency** (re-proven every cycle, not a one-off snapshot) and the blake2b **proof-of-reasoning**. Then tamper with the inputs and watch both proofs fail. Two independent implementations of the same maths — [`web/lib/zk-verify.ts`](web/lib/zk-verify.ts) (browser) and [`contracts/src/zk_reserves.rs`](contracts/src/zk_reserves.rs) (on-chain).
...
plus **attestation**, **x402 settlement**, **reputation** (authority-gated
...
two keys per cycle, neither able to forge the other's.
**The vault enforces this itself — it is not a convention of the agent's own code.**
`RwaVault.reallocate` calls `AuditorQuorum.approved(attestation_hash)` and **reverts
`NotApproved`** unless a **K-of-N quorum of independent auditors has signed APPROVE for
that exact decision on-chain**. Proven live: with the agent's own key signing and its
reputation passing, a decision the auditors never approved was still **refused by the
contract** ([`ba368de3…`](https://testnet.cspr.live/deploy/ba368de335840645486c7692cf1fdee8b0ca3f7f61514091515a32052ac2d7b7)),
while the quorum-approved one executed ([`e68d4218…`](https://testnet.cspr.live/deploy/e68d42184b6f7fac2e226bea10c6a3e0942a276da6d6065618ac0f2d6c533c8e)).
So even a **fully compromised agent key cannot move funds** without the auditors.
**Skin in the game:** a veto also **slashes the agent's on-chain reputation** — and both
`adjust` *and* `record_payment` are **authority-only**, so the agent can neither inflate
its own score nor farm reputation to walk past the circuit breaker.
...
ingest live RWA prices  →  enrich via the official CSPR.cloud + CSPR.trade MCP servers
  ...
  →  K-of-N AUDITOR QUORUM: each independent auditor signs APPROVE for this decision
       hash and votes on-chain (AuditorQuorum, 2-of-3)
  →  SpendGate.check + ComplianceRegistry.assert_valid (custodian-owned gates)
  →  RwaVault.reallocate (yield only, $800K principal locked) — THE CONTRACT re-checks
       AuditorQuorum.approved(hash) and reverts NotApproved, plus the reputation floor
       and frozen (dead-man's switch) circuit breakers
```
...
| ZkReserves (on-chain ZK proof-of-reserves — Pedersen+Schnorr; the proof carries only the total) | `5b84d2f911d4bed7e7345c22a0236794b5dc8033f3fb8870595b0fb6e8f3688a` |
