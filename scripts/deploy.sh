...
# (agent/src/deploy.ts). That is what produced the live deployment, needs no
# casper-client/jq, and stays in sync. This casper-client script DIVERGES (omits
# PaymentToken's chain_name init arg, uses 300 CSPR / an older key name) and would
# MIS-DEPLOY the x402 token — kept only as a casper-client reference.
#
...
# x402 payment asset: PaymentToken (local CEP-18 wrapper). Its package hash becomes
# X402_ASSET_PACKAGE_HASH in signal-service. init mints initial_supply (1M @ 6dp).
deploy_one X402_ASSET_PACKAGE_HASH PaymentToken.wasm amanah_payment_token_package_hash \
