// Vault v8 — fixes a VALUE-CREATION bug found by edge-case testing.
//
...
const STATE = resolve(import.meta.dirname, "../../.env.vaultv8");
const VAULT_KEY = "amanah_vault_v8_package_hash";
...
const QUORUM = "2663d7ce209f999670be56dc2732512cd500f1cd4423f1623383fff68ff3dfeb";     // v2 (2-of-3)
...
async function reallocate(from: number, to: number, attHash: string) {
  try {
    const { deployHash } = await callEntryPoint({
      rpc, key: agentKey, contractHash: state.VAULT_V8, entryPoint: "reallocate",
      args: Args.fromMap({
...
async function main() {
  ...
  console.log("deploying vault v8 (SameAsset value-conservation fix)\n");
  ...
  if (!state.VAULT_V8) {
    ...
    console.log("  install vault v8:", dh, "— waiting...");
    ...
    save("VAULT_V8", nk.key.toPrefixedString().replace(/^.*-/, ""));
    console.log("  VAULT_V8 =", state.VAULT_V8);
  }
  ...
  if (!state.SET_SPENDER) {
    const { deployHash } = await callEntryPoint({ rpc, key: custodian, contractHash: SPENDGATE, entryPoint: "set_spender",
      args: Args.fromMap({ spender: CLValue.newCLKey(Key.newKey(`hash-${state.VAULT_V8}`)) }), chainName: config.chainName, paymentMotes: config.paymentMotes });
    console.log("  set_spender(vault v8):", deployHash); save("SET_SPENDER", deployHash);
  }
  ...
  for (const [name, idx, amt] of seeds) {
    ...
    const { deployHash } = await callEntryPoint({ rpc, key: agentKey, contractHash: state.VAULT_V8, entryPoint: "deposit",
  ...
  if (!state.HEARTBEAT) {
    const { deployHash } = await callEntryPoint({ rpc, key: agentKey, contractHash: state.VAULT_V8, entryPoint: "heartbeat",
  ...
  console.log("\n=== VAULT v8 LIVE (instance-bound quorum) ===");
  console.log("VAULT_V8         :", state.VAULT_V8);
